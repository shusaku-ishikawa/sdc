...
class MyDecoder:
    ...
    def decode(self, doesInvert):
        ...
        pix_per_mm_x = width_in_pix / self.oven.floor_width_in_mm
        pix_per_mm_y = height_in_pix / self.oven.floor_height_in_mm
        ...
        for ch in channels:
            ...
            for barcode in barcodes:
                ...
                for point in barcode.polygon:
                    if self._find_point(ch["x_offset"]*pix_per_mm_x, ch["y_offset"]*pix_per_mm_y, ch["x_offset"]*pix_per_mm_x + ch["width"]*pix_per_mm_x, ch["y_offset"]*pix_per_mm_y + ch["height"]*pix_per_mm_y, point.x, point.y):
                        partialIn = True
                #print(str(ch['id']) + "::" + barcode.data.decode('utf-8'))
                if partialIn:
                    if 'qr' in temp.keys():
                        return self.MULTIPLE_PRODUCTS_IN_CHANNEL
                    else:
                        temp['qr'] = barcode.data.decode('utf-8')
            self._draw_rectangle((top_left, bottom_right), color = 'red', width = 10)
            result.append(temp)
        return result
